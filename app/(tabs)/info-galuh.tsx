...
export default function InfoGaluhScreen() {
  ...
  const [kontakDaruratList, setKontakDaruratList] = useState([]);
  const [bannerHighlightList, setBannerHighlightList] = useState([]);
  ...
  const fetchInfoGaluhLiveData = async (isInitialLoad = false) => {
    try {
      if (isInitialLoad) setLoading(true); 
      
      const [resBeritaWeb, resBeritaGaluh, resLoker, resWisata] = await Promise.all([
        supabase.from('berita_website').select('*').eq('status', 'published').order('created_at', { ascending: false }).limit(5), // 🟢 Banner Sorotan Atas
        supabase.from('berita_galuh').select('*').order('created_at', { ascending: false }).limit(10), // 🟢 Kabar Harian Bawah
        supabase.from('loker_galuh').select('*').eq('is_valid', true).order('created_at', { ascending: false }), // 🟢 Loker
        supabase.from('wisata_galuh').select('*').order('nama_destinasi', { ascending: true }) // 🟢 Wisata
      ]);

      // Mapping Berita Banner Atas (dari berita_website)
      if (!resBeritaWeb.error && resBeritaWeb.data) {
        const bannerData = resBeritaWeb.data.map(b => ({
          id_berita: b.id,
          judul_berita: b.judul,
          isi_berita: b.konten,
          gambar_berita: b.gambar_thumbnail
        }));
        setBannerHighlightList(bannerData);
      }

      // Mapping Kabar Ciamis Bawah (dari berita_galuh)
      if (!resBeritaGaluh.error && resBeritaGaluh.data) {
        const beritaData = resBeritaGaluh.data.map(b => ({
          id_berita: b.id,
          judul_berita: b.judul,
      ...
      // Mapping Loker Galuh
      if (!resLoker.error && resLoker.data) {
        const lokerData = resLoker.data.map(l => ({
          id: l.id,
        ...
        setLokerList(lokerData);
      }

      // Mapping Wisata Galuh
      if (!resWisata.error && resWisata.data) {
        const wisataData = resWisata.data.map(w => ({
          id: w.id,
  ...
  useFocusEffect(
    useCallback(() => {
      if (isFirstMount.current) {
        fetchInfoGaluhLiveData(true);
        isFirstMount.current = false;
      } else {
        fetchInfoGaluhLiveData(false); 
      }
    }, [])
  );

  const handlePilihKecamatanDarurat = async (namaKecamatan) => {
    const kecamatanBersih = namaKecamatan.trim();
    setSelectedKecamatan(kecamatanBersih);
    setKontakDaruratList([]);
    
    try {
      setLoadingDarurat(true);
      // 🟢 Menembak Tabel Darurat yang Baru
      const { data, error } = await supabase.from('darurat_galuh').select('*').eq('kecamatan', kecamatanBersih); 
      if (!error && data) setKontakDaruratList(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleHubungiNomor = (nomor, tipe = 'tel') => {
    if (!nomor) return;
    if (tipe === 'wa') {
  ...
  const getIconKategoriRescue = (kategori) => {
  ...
  const getColorKategoriRescue = (kategori) => {
            ...
            <View style={styles.subSectionBlock}>
              ...
              {beritaList.length === 0 ? <View style={styles.emptyContainer}><Text style={styles.emptyText}>Tidak ada kabar atau pengumuman hari ini.</Text></View> : beritaList.map(berita => (
                <View key={berita.id_berita} style={styles.beritaCard}>
                  {/* 🟢 Warna & Ikon Dinamis dari Supabase */}
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { paddingBottom: insets.bottom + 20 }]}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => { setModalDaruratVisible(false); setSelectedZona(null); setSelectedKecamatan(null); setKontakDaruratList([]); }}><Ionicons name="close-circle" size={24} color="#C62828" /></TouchableOpacity>
            </View>
              ...
              {!selectedZona ? (
                ...
                <View>
                  ...
                  <View style={styles.kecamatanGrid}>
                    {selectedZona.kecamatan.map((kec) => (<TouchableOpacity key={kec} style={styles.kecamatanBox} onPress={() => handlePilihKecamatanDarurat(kec)}><Text style={styles.kecamatanText}>{kec}</Text></TouchableOpacity>))}
                  </View>
                </View>
              ) : (
                <View style={{ flex: 1 }}>
                  <TouchableOpacity style={styles.btnBackModal} onPress={() => { setSelectedKecamatan(null); setKontakDaruratList([]); }}><Ionicons name="arrow-back" size={14} color="#8D6E63" /><Text style={styles.btnBackModalText}>Kembali ke Pilihan Kecamatan</Text></TouchableOpacity>
                  <Text style={styles.daruratHeaderTarget}>📍 Unit Penyelamat Kec. {selectedKecamatan}</Text>
                  {loadingDarurat ? (
                    <View style={{ paddingVertical: 40, justifyContent: 'center', alignItems: 'center' }}><ActivityIndicator size="small" color="#4A3525" /><Text style={{ fontSize: 11, color: '#8D6E63', marginTop: 8 }}>Memanggil unit penyelamat terdekat...</Text></View>
                  ) : (
                    <View style={{ gap: 10, marginTop: 10, paddingBottom: 20 }}>
                      {kontakDaruratList.length === 0 ? (
                        <View style={styles.emptyContainer}><Text style={styles.emptyText}>Nomor darurat khusus kecamatan ini belum diinput di database.</Text></View>
                      ) : (
                        kontakDaruratList.map((rescue, index) => (
                          <View key={rescue.id || String(index)} style={styles.kontakRescueCard}>
                            <View style={styles.rescueHeaderRow}><Text style={styles.rescueName}>{rescue.nama_instansi}</Text><Ionicons name={getIconKategoriRescue(rescue.kategori)} size={14} color={getColorKategoriRescue(rescue.kategori)} /></View>
                            <TouchableOpacity style={[styles.btnCallRescue, { backgroundColor: getColorKategoriRescue(rescue.kategori) }]} onPress={() => handleHubungiNomor(rescue.nomor_telepon)}><Ionicons name="call" size={12} color="#fff" style={{ marginRight: 6 }} /><Text style={styles.btnCallRescueText}>Hubungi Bantuan</Text></TouchableOpacity>
                          </View>
                        ))
                      )}
                    </View>
