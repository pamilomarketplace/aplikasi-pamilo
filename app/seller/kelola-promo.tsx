...
export default function KelolaPromoScreen() {
  ...
  // 1 = Kupon Voucher Toko | 2 = Minimal Belanja (Opsi Potong Harga dicabut agar seragam di Checkout)
  const [tipePromoTerpilih, setTipePromoTerpilih] = useState<1 | 2>(1);
  ...
  const handleSimpanPromoLapak = async () => {
    ...
    try {
      ...
      if (tipePromoTerpilih === 1 && !kodeVoucher.trim()) {
      ...
      const { error } = await supabase
        .from('promosi_toko')
        .insert([{
          id_toko_promo: idToko, 
          jenis_promo: tipePromoTerpilih === 1 ? 'VOUCHER_KODE' : 'MIN_BELANJA',
          kode_promo: tipePromoTerpilih === 1 ? kodeVoucher.trim().toUpperCase() : null,
          tipe_potongan: tipePotongan, 
          nilai_potongan: parseInt(nilaiPotongan),
          minimal_belanja: parseInt(minBelanja || '0'), 
          kuota_promo: parseInt(kuota || '100')
        }]);

      if (error) throw error;
      
      Alert.alert("Sukses Terpasang! 🎉", tipePromoTerpilih === 1 ? `Kode Kupon Voucher "${kodeVoucher.toUpperCase()}" aktif mengudara.` : "Diskon otomatis minimum belanja berhasil diterapkan di lapak Tuan.");
  ...
  const handleHapusPromo = async (id_promo: string) => {
    const res = await new Promise(resolve => {
        Alert.alert("Hapus Promo?", "Promo ini akan ditarik dari peredaran dan tidak bisa digunakan lagi oleh warga.", [
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 20 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.tabContainer}>
          <TouchableOpacity style={[styles.tabButton, tipePromoTerpilih === 1 && styles.tabButtonActive]} onPress={() => setTipePromoTerpilih(1)}>
            <FontAwesome5 name="ticket-alt" size={11} color={tipePromoTerpilih === 1 ? '#fff' : '#4A3525'} />
            <Text style={[styles.tabText, tipePromoTerpilih === 1 && styles.tabTextActive]}>Kupon Toko</Text>
          </TouchableOpacity>

          <TouchableOpacity style={[styles.tabButton, tipePromoTerpilih === 2 && styles.tabButtonActive]} onPress={() => setTipePromoTerpilih(2)}>
            <FontAwesome5 name="shopping-basket" size={11} color={tipePromoTerpilih === 2 ? '#fff' : '#4A3525'} />
            <Text style={[styles.tabText, tipePromoTerpilih === 2 && styles.tabTextActive]}>Min Belanja</Text>
          </TouchableOpacity>
        </View>

        <View style={styles.memoBox}>
          <Text style={styles.memoText}>
            ℹ️ {tipePromoTerpilih === 1 && "Opsi Kupon Toko: Warga wajib mengetikkan kode unik kupon ini saat checkout untuk memicu diskon."}
            {tipePromoTerpilih === 2 && "Opsi Min Belanja: Diskon memotong tagihan warga secara otomatis jika total belanjaan di lapak Tuan menyentuh batas limit."}
          </Text>
        </View>
                    ...
                    <View key={promo.id} style={styles.promoItemBox}>
                        <View style={{flex: 1}}>
                            ...
                            <Text style={styles.promoItemDesc}>
                                Potongan: <Text style={{fontWeight:'bold', color:'#D35400'}}>{promo.tipe_potongan === 'NOMINAL' ? `Rp ${promo.nilai_potongan.toLocaleString()}` : `${promo.nilai_potongan}%`}</Text> • Total Kuota Tersedia: {promo.kuota_promo}
                            </Text>
